  ...
  try {
    ...
    foreach ($events as $index => $event) {
      ...
      if ($event_date == $three_days_after) {
        $stmt = $db->prepare('SELECT users.name, users.email, event_attendance.status FROM event_attendance left join users on event_attendance.user_id = users.id right join events on event_attendance.event_id = events.id where status = 1 & events.id = :event_id');
